...
| **E**levation of privilege | Malicious image | `--pull=never` — the image must already be present, so nothing is fetched at task time | **No signature or digest verification.** Pin by digest and verify upstream |
