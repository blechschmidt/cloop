...
// RemovePID removes the PID file (called by worker on clean exit). Returns
// the underlying os.Remove error so callers can log a true failure (perms,
// busy filesystem) — a stale PID file blocks the next `daemon start` because
