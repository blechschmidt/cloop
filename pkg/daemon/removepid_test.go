...
// left behind and the next `daemon start` sees IsRunning() return true and
// refuses to launch. The user has to manually `rm .cloop/daemon.pid` with no
// log to point them at it. Returning the error lets callers log the failure
// so the cause is at least visible in the daemon log.
...
func TestRemovePID_ReturnsNilWhenFileExisted(t *testing.T) {
	tmp := t.TempDir()
	if err := daemon.WritePID(tmp, 4242); err != nil {
		t.Fatalf("seeding PID file: %v", err)
	}
	if err := daemon.RemovePID(tmp); err != nil {
		t.Fatalf("RemovePID after WritePID should succeed, got %v", err)
	}
	if _, statErr := os.Stat(daemon.PIDPath(tmp)); !os.IsNotExist(statErr) {
