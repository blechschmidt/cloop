...
// readiness check can stat + open + ping it. The file remains on disk after
// the helper closes its handle.
...
// newProbeHandler builds the apiserver's HTTP handler chain for httptest.
// It mirrors the construction in Run() so middleware (rate-limit + auth)
// and the probe bypass behave exactly as in production.
func newProbeHandler(s *Server) http.Handler {
	mux := http.NewServeMux()
	// Add a representative protected route so middleware bypass tests can
	// assert real-world behaviour. Tests that only exercise probes never
	// hit this endpoint, so its handler is intentionally trivial.
...
// TestHealthz_AlwaysOK verifies /healthz returns 200 unconditionally. This is
// the load-bearing K8s liveness invariant — a degraded DB MUST NOT cause the
// orchestrator to kill an otherwise-recoverable process.
func TestHealthz_AlwaysOK(t *testing.T) {
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			newProbeHandler(s).ServeHTTP(rr, req)
...
// TestReadyz verifies /readyz reflects the SQLite store's actual reachability:
// 200 when state.db is present and pings successfully, 503 (with a JSON body
// explaining the failed check) otherwise.
