...
// FormatMarkdown renders the standup as GitHub-flavored Markdown.
func FormatMarkdown(r *Report, goal string, windowHours int) string {
	var b strings.Builder
	...
	m := r.Metrics
	b.WriteString(fmt.Sprintf("**Progress:** %d%% (%d/%d tasks) | **Velocity:** %.1f/day | **Risk:** %s (%d/100)\n\n",
		m.CompletionPct(), m.DoneTasks, m.TotalTasks, m.VelocityPerDay, m.RiskLabel(), m.RiskScore))
	b.WriteString("---\n\n")
	b.WriteString(r.AIText)
	b.WriteString("\n\n---\n\n")
